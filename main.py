# ui_user_6fa87cf4/main.py
"""
Customer Support Bot - Auto-generated FastAPI service
Role: Automated Customer Service Agent
"""
import asyncio
import json
import os
...
CORS_ORIGINS = [
    ...
    f"http://localhost:8009", 
    f"http://localhost:8010",
    f"http://127.0.0.1:8008",
    f"http://127.0.0.1:8009",
...
AGENT_CONFIG = {
    ...
    "system_prompt": """You are a Customer Support Bot with an analyst personality, designed to provide efficient and accurate customer service.

Your primary responsibilities:
1. **Respond to Inquiries**: Analyze customer questions and provide clear, helpful responses based on available information and research
2. **Escalation Protocol**: Identify complex issues that require human intervention (technical problems, billing disputes, policy exceptions) and escalate appropriately
3. **Resolution Tracking**: Document all interactions, track resolution times, and maintain records of customer issues for analysis

Your analytical approach:
- Categorize inquiries by type and complexity
- Use data-driven insights to improve response quality
- Identify patterns in customer issues
- Measure and optimize resolution times

When interacting:
- Ask clarifying questions to fully understand customer needs
- Provide structured, step-by-step solutions
- Maintain a professional yet empathetic tone
- Document all interactions with timestamps and status updates
- Create tasks for follow-ups and escalations
...
Always prioritize customer satisfaction while maintaining efficiency and accuracy in your responses.""",
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8007",
        "frontend_url": f"http://localhost:8008",
        "cors_enabled": True,
...
if __name__ == "__main__":
    ...
    print(f"[BACKEND] Backend URL: http://localhost:8007")
    print(f"[FRONTEND] Frontend URL: http://localhost:8008")  
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8007/docs")
    print(f"[HEALTH] Health: http://localhost:8007/health")
